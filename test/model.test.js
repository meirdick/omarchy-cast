...
// Three physical devices, all of them playing: the Google TV's two protocols
// are one row, so there is no idle section at all here.
eq("a header, the devices, then the note",
   kinds, "header,device,device,device,note")
eq("playing section first", rows[0].label, "Playing")
eq("three devices", rows.filter(function (r) {
...
eq("cannot walk off the end", M.moveCursor(rows, 3, 1), 3)
eq("cannot walk off the start", M.moveCursor(rows, 1, -1), 1)
eq("empty rows are survivable", M.moveCursor([], 0, 1), 0)

// Header skipping needs its own case now that the real fixture happens to
// produce only one section.
var twoSections = [
  { kind: "header", selectable: false },
  { kind: "device", selectable: true },
  { kind: "header", selectable: false },
  { kind: "device", selectable: true },
  { kind: "note", selectable: false }
]
eq("moving down skips the second header", M.moveCursor(twoSections, 1, 1), 3)
eq("moving up skips it too", M.moveCursor(twoSections, 3, -1), 1)
eq("the trailing note never takes the cursor", M.moveCursor(twoSections, 3, 1), 3)
eq("clamping past the end lands on a device", M.clampCursor(twoSections, 4), 3)
